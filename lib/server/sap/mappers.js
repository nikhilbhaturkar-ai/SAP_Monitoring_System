 ...
 */
// Z_STRUST_SRV returns one array with three tag groups per call; each of the
// three STRUST checks reads its own group by this TAG.
const STRUST_TAG_BY_CHECK = {
  strust: 'Already Expired',
  strustToday: 'Expiring Today',
  strust15d: 'Expiring in 15 Days',
};
...
export function toRawValue(checkKey, payload) {
  switch (checkKey) {
    ...
    case 'cancel': {
      // Z_CANCEL_JOBS: an array of cancelled-job rows (NEWFLAG = 'C'). The
      // tile shows the count; the rows themselves are separately captured by
      // toCancelJobDetails() below.
      const count = Array.isArray(payload) ? payload.length : null;
      if (count == null) return null;
      return count === 0 ? 'No cancelled jobs found' : `${count} cancelled job(s) found`;
    }

    case 'strust':
    case 'strustToday':
    case 'strust15d': {
      // Z_STRUST_SRV: a top-level array of tag groups ({ TAG,
      // NO_OF_CERTIFICATES, CERT_DETAILS }) — one API call, three checks each
      // reading their own group's count as a plain numeric string, same as
      // st22. normal_text is "0" for all three.
      const groups = Array.isArray(payload) ? payload : [];
      const group = groups.find((g) => g?.TAG === STRUST_TAG_BY_CHECK[checkKey]);
      const count = Number(group?.NO_OF_CERTIFICATES);
...
/**
 * Long-running job list behind an sm37 observation's headline count.
 * Z_SM37_JOBS_SRV: { NO_OF_LONG_RUNNING_JOBS, LONG_RUNNING_JOBLIST: [{
 * JOBNAME, SDLSTRTDT, SDLSTRTTM, … }] }.
 *
 * @returns {Array<object>|null}
 */
export function toJobDetails(checkKey, payload) {
  if (checkKey !== 'sm37') return null;
  const jobs = payload?.LONG_RUNNING_JOBLIST;
  if (!Array.isArray(jobs)) return null;

  return jobs.map((row) => ({
    jobName: row?.JOBNAME ?? null,
    schedDate: row?.SDLSTRTDT ?? null,
    schedTime: row?.SDLSTRTTM ?? null,
  }));
}

/**
 * Cancelled-job list behind a cancel observation's headline count.
 * Z_CANCEL_JOBS: a top-level array of job rows, one per cancelled job
 * (NEWFLAG = 'C'); JOBNAME, SDLSTRTDT, SDLSTRTTM, RELUNAME, NEWFLAG, JOBLOG,
 * WPPROCID, BTCSYSREAX, REAXSERVER are the fields the click-through table
 * uses — the rest (JOBCOUNT, STEPCOUNT, timestamps, …) aren't shown.
 *
 * @returns {Array<object>|null}
 */
export function toCancelJobDetails(checkKey, payload) {
  if (checkKey !== 'cancel') return null;
  if (!Array.isArray(payload)) return null;
  ...
  return payload.map((row) => ({
    ...
    schedDate: row?.SDLSTRTDT ?? null,
    schedTime: row?.SDLSTRTTM ?? null,
    userName: row?.RELUNAME ?? null,
    status: row?.NEWFLAG ?? null,
    jobLog: row?.JOBLOG ?? null,
    wpProcess: row?.WPPROCID != null ? String(row.WPPROCID) : null,
    btcsysreax: row?.BTCSYSREAX ?? null,
    reaxserver: row?.REAXSERVER ?? null,
  }));
}

/**
 * Certificate list behind one of the three STRUST observations' headline
 * count. Z_STRUST_SRV: a top-level array of tag groups; each check's own
 * group's CERT_DETAILS is stored, matching the count toRawValue reports.
 *
 * @returns {Array<object>|null}
 */
export function toCertDetails(checkKey, payload) {
  const tag = STRUST_TAG_BY_CHECK[checkKey];
  if (!tag) return null;
  const groups = Array.isArray(payload) ? payload : [];
  const group = groups.find((g) => g?.TAG === tag);
  const certs = group?.CERT_DETAILS;
...
/**
 * Error-queue rows behind an smq1/smq2 observation's headline count.
 * Z_SMQ1_OUTB_SRV / Z_SMQ2_INB_SRV: { NO_OF_ERROR_QUEUES, ERROR_QTABLE: [{
 * QNAME, DEST, QSTATE, QRFCUSER, QRFCDATUM, QRFCUZEIT, ERRMESS, … }] }. One
 * function for both checks — they return the identical row shape, only the
 * direction (outbound/inbound) differs.
 *
 * @returns {Array<object>|null}
 */
export function toQueueErrorDetails(checkKey, payload) {
  if (checkKey !== 'smq1' && checkKey !== 'smq2') return null;
  const rows = payload?.ERROR_QTABLE;
  if (!Array.isArray(rows)) return null;

  return rows.map((row) => ({
    arfcipid: row?.ARFCIPID ?? null,
    arfcpid: row?.ARFCPID ?? null,
    queueName: row?.QNAME ?? null,
    destination: row?.DEST ?? null,
    queueState: row?.QSTATE ?? null,
    rfcUser: row?.QRFCUSER ?? null,
    rfcFunction: row?.QRFCFNAM ?? null,
    rfcDate: row?.QRFCDATUM ?? null,
    rfcTime: row?.QRFCUZEIT ?? null,
    errorMessage: row?.ERRMESS ?? null,
  }));
}

/**
 * Audit-file rows behind an sm20 observation's OUTPUT sentence.
 * Z_SM20_LOG_SRV: { OUTPUT, AUDIT_FILE: [{ INSTNAME, FILENAME, RECCNT,
 * RECGOOD, MAXSIZE, ENDREASON, … }] }.
 *
 * @returns {Array<object>|null}
 */
export function toAuditFileDetails(checkKey, payload) {
  if (checkKey !== 'sm20') return null;
  const files = payload?.AUDIT_FILE;
  if (!Array.isArray(files)) return null;
  const outputText = typeof payload?.OUTPUT === 'string' ? payload.OUTPUT : null;

  return files.map((row) => ({
    instanceName: row?.INSTNAME ?? null,
    fileName: row?.FILENAME ?? null,
    recordCount: row?.RECCNT != null ? Number(row.RECCNT) : null,
    recordGood: row?.RECGOOD != null ? Number(row.RECGOOD) : null,
    maxSize: row?.MAXSIZE != null ? Number(row.MAXSIZE) : null,
...
/**
 * Pending tRFC list behind an sm58 observation's headline count.
 * Z_SM58_TRFC_SRV: { NO_OF_TRFCS, ARFCISTATE: [{ ARFCIPID, ARFCDEST,
 * ARFCFNAM, ARFCTCODE, ARFCRHOST, ARFCMSG, ARFCRESERV, HASH, … }] }.
 *
 * @returns {Array<object>|null}
 */
export function toTrfcDetails(checkKey, payload) {
  if (checkKey !== 'sm58') return null;
  const rows = payload?.ARFCISTATE;
  if (!Array.isArray(rows)) return null;

  return rows.map((row) => ({
    arfcipid: row?.ARFCIPID ?? null,
    arfcdest: row?.ARFCDEST ?? null,
    arfcfnam: row?.ARFCFNAM ?? null,
    arfctcode: row?.ARFCTCODE ?? null,
    arfcrhost: row?.ARFCRHOST ?? null,
    arfcmsg: row?.ARFCMSG ?? null,
    arfcreserv: row?.ARFCRESERV ?? null,
    hash: row?.HASH ?? null,
  }));
}

/**
 * Syslog entry list behind an sm21 observation's headline count.
 * Z_SM21_LOG_SRV: { NO_OF_SYSLOG_MSG, SYSLOG_ENTRIES: [{ ZDATE, ZTIME,
 * INSTANCE, ICON, TEXT, SLGDATA, … }] }.
 *
 * @returns {Array<object>|null}
 */
export function toSyslogDetails(checkKey, payload) {
  if (checkKey !== 'sm21') return null;
  const entries = payload?.SYSLOG_ENTRIES;
  if (!Array.isArray(entries)) return null;

  return entries.map((row) => ({
    logDate: row?.ZDATE ?? null,
    logTime: row?.ZTIME ?? null,
    instanceName: row?.INSTANCE ?? null,
    priorityIcon: row?.ICON ?? null,
    messageText: row?.TEXT ?? null,
    slgData: row?.SLGDATA || null,
  }));
}
...
export function toWorkerDetails(checkKey, payload) {
  ...
  return workers.map((row) => ({
    ...
    wpCpu: row?.CPU ?? null,
    wpPid: row?.PID != null ? Number(row.PID) : null,
    waitPriority: row?.WAIT_FOR_PRIORITY_DISP || null,
  }));
}
