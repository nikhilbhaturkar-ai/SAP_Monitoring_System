...
export function toRawValue(checkKey, payload) {
  switch (checkKey) {
    ...
    case 'strust': {
      ...
      // NO_OF_CERTIFICATES, CERT_DETAILS }). normal_text is "0", so the
      // headline is the "Already Expired" group's count as a plain numeric
      // string, same as st22.
      const groups = Array.isArray(payload) ? payload : [];
      const expired = groups.find((g) => g?.TAG === 'Already Expired');
      const count = Number(expired?.NO_OF_CERTIFICATES);
...
/**
 * Expired-certificate list behind a strust observation's headline count.
 * Z_STRUST_SRV: a top-level array of tag groups; only the "Already Expired"
 * group's CERT_DETAILS is stored, matching the count toRawValue reports.
 *
 * @returns {Array<object>|null}
 */
export function toCertDetails(checkKey, payload) {
  if (checkKey !== 'strust') return null;
  const groups = Array.isArray(payload) ? payload : [];
  const expired = groups.find((g) => g?.TAG === 'Already Expired');
  const certs = expired?.CERT_DETAILS;
