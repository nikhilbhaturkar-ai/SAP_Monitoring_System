...
export async function processAlertNotifications() {
  ...
  const ignoreDeduplication = settings['ignore_deduplication'] === 'true';
  ...
  console.log(`> [Notifier] Deduplication check: ${ignoreDeduplication ? 'DISABLED (sending emails on every worker run)' : 'ENABLED (filtering duplicate alerts)'}`);
  ...
  for (const user of userRows) {
    ...
    // Check deduplication in sent_notifications table unless ignoreDeduplication is TRUE
    ...
    for (const alert of userAnomalies) {
      ...
      // New alert, changed status, or ignoreDeduplication is enabled
