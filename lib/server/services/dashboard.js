 ...
 */
function healthScore(anomalyCount) {
  return Math.max(84, 100 - anomalyCount * 6);
}
...
const ALWAYS_CRITICAL_CHECKS = new Set(['st22', 'sm51', 'sm13', 'sm50']);
...
// reachable by clicking regardless of whether the latest run passed.
const ALWAYS_OPENABLE_CHECKS = new Set(['sm51', 'sm12', 'st06', 'backup', 'sm50']);
...
export async function buildDashboard(sid, windowDays = 20) {
  ...
  const [
    ...
    byKey.get('strust')?.is_anomaly
      ? repo.getLatestCerts(sid).then((rows) =>
          rows.map((c) => ({
  ...
  const params = latestObs
    ...
    .map((o) => ({
      ...
      label: o.check_label,
      value: o.raw_value ?? '—',
      status: o.is_anomaly ? 'warning' : o.is_info && !ALWAYS_OPENABLE_CHECKS.has(o.check_key) ? 'info' : 'ok',
  ...
  return {
    ...
    landscape: landscape.map((row) => ({
      ...
      score: healthScore(Number(row.window_alerts)),
    })),
  };
}
...
export async function buildHistory(sid, limit = 60) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      note: count === 0 ? 'Normal' : row.anomaly_values.join('; '),
