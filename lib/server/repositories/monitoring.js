...
export async function getLatestCerts(sid, checkKey) {
  const { rows } = await query(
...
/** SM21 syslog entries for one system's latest run. */
export async function getLatestSyslogEntries(sid) {
  const { rows } = await query(
    `SELECT l.log_date, l.log_time, l.instance_name, l.priority_icon, l.message_text, l.slg_data
       FROM syslog_entries l
       JOIN systems s    ON s.id = l.system_id
       JOIN daily_runs r ON r.id = l.run_id
      WHERE s.sid = $1
        AND r.run_date = (SELECT max(run_date) FROM daily_runs)
      ORDER BY l.log_date DESC NULLS LAST, l.log_time DESC NULLS LAST, l.id`,
    [sid]
  );
  return rows;
}

/** SM58 pending tRFC list for one system's latest run. */
export async function getLatestTrfcs(sid) {
  const { rows } = await query(
    `SELECT t.arfcipid, t.arfcdest, t.arfcfnam, t.arfctcode, t.arfcrhost,
            t.arfcmsg, t.arfcreserv, t.trfc_hash
       FROM sm58_trfcs t
       JOIN systems s    ON s.id = t.system_id
       JOIN daily_runs r ON r.id = t.run_id
      WHERE s.sid = $1
        AND r.run_date = (SELECT max(run_date) FROM daily_runs)
      ORDER BY t.id`,
    [sid]
...
export async function getLandscapeSummary(windowDays = 20) {
  const { rows } = await query(
    `WITH latest_per_system AS (
       SELECT o.system_id, MAX(dr.run_date) AS latest_run_date
         FROM observations o
     ...
     SELECT s.sid,
            s.name,
            COUNT(*) FILTER (
              WHERE o.is_anomaly AND wr.run_date = lps.latest_run_date
            ) AS open_alerts,
            COUNT(*) FILTER (WHERE o.is_anomaly) AS window_alerts
       FROM daily_runs wr
       JOIN observations o ON o.run_id = wr.id
       JOIN systems s      ON s.id = o.system_id
       JOIN latest_per_system lps ON lps.system_id = s.id
      WHERE s.kind = 'sap'
